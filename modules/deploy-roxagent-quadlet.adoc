...
. Create the `roxagent-prep.service` file:
+
[source,terminal]
----
$ sudo tee /etc/systemd/system/roxagent-prep.service << 'EOF'
[Unit]
...
. Create the `roxagent.timer` file:
+
[source,terminal]
----
$ sudo tee /etc/systemd/system/roxagent.timer << 'EOF'
[Unit]
...
. Create the `roxagent.container` file, replacing `<version>` with the version of your {product-title-short} deployment:
+
[source,terminal]
----
$ sudo tee /etc/containers/systemd/roxagent.container << 'EOF'
[Unit]
